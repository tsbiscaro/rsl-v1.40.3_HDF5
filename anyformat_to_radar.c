...
enum File_type RSL_filetype(char *infile)
{
  ...
  if (fread(magic, sizeof(magic), 1, fp) != 1) {
	...
	perror("RSL_filetype");
        /*
        All I/O streams opened with popen MUST
        be closed with pclose - see 'man pclose'
        */
        rsl_pclose(fp);
	return UNKNOWN;
  }
  ...
  */
  ...
  /*Added CTH test*/
  if (strncmp("SPPeRiHad", &magic[1], 9) == 0) return CTH_FILE;
  if (strncmp("PPS HiRes", &magic[0], 9) == 0) return CTH_FILE;
  /*Added HDF5 test...*/
  if ((int)magic[1] == 0x48 && (int)magic[2] == 0x44 &&
      (int)magic[3] == 0x46 && (int)magic[4] == 0x0d &&
      (int)magic[5] == 0x0a && (int)magic[6] == 0x1a &&
      (int)magic[7] == 0x0a) return HDF5_FILE;
...
//  if (strncmp("begin( volume )", &magic[2], 15) == 0) return DSA_GEMATRONIK_FILE;
