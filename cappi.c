...
int RSL_fill_cappi(Volume *v, Cappi *cap, int method)
   ...
   Ray *ray;
   Sweep *sweep;
   int ini_bins, end_bins;
   int total_bins, dbins;
...
#if 0
   /*
   fill cappi with PPI data to avoid blank areas (pseudo-cappi)
   Thiago Biscaro - 13.SEP.2011
   */
   
   total_bins = cap->sweep->ray[0]->h.nbins;
   dbins = total_bins / v->h.nsweeps;
   
   for (a = v->h.nsweeps - 1; a > -1; a--)
      {
      ini_bins = (v->h.nsweeps - a)*dbins;
      end_bins = (v->h.nsweeps - a + 1)*dbins;

      /*filling the cappi border - adjusting the
      loop limit to account for possible rounding errors*/
      if (a == 0)
         end_bins = total_bins;
      
      for(b=0; b < cap->sweep->h.nrays; b++)
         {
         ...
         for(c = ini_bins; (c < end_bins && c < ray->h.nbins); c++)
            {
            cap->sweep->ray[b]->range[c] = ray->range[c];
            }
         }
      }
